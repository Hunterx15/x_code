...
const SubmissionHistory = ({ problemId }) => {
  ...
  useEffect(() => {
    // Bug #30 fix: add cancellation flag so a slow in-flight fetch doesn't
    // overwrite the state if the user has already navigated to a different problem.
    let cancelled = false;
  ...
  const formatMemory = (memory) => {
    // Bug #36 fix: guard against undefined/null/NaN (backend may omit the field).

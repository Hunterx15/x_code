...
const submitCode = async (req,res)=>{
    ...
    //    Bug #16 fix: null-check the problem before accessing hiddenTestCases.
       if(!problem)
    ...
    // Batch I: After an accepted submission, check for newly-earned badges.
    // Fire-and-forget (don't block the submission response on badge logic),
    // but await so any errors are caught by the outer try/catch.
    ...
    if (status === 'accepted') {
      try {
        ...
        console.error('Badge award check failed:', e);
      }
    }
    ...
    res.status(201).json({
      ...
      runtime,
      memory,
      // Batch I: include newly-awarded badges so the frontend can show a toast
...
const runCode = async(req,res)=>{
   ...
   //    Fetch the problem from database
      const problem =  await Problem.findById(problemId);
   //    Bug #16 fix: null-check the problem before accessing visibleTestCases.
      if(!problem)
       return res.status(404).send("Problem not found");
   //    testcases(Hidden)
      if(language==='cpp')
